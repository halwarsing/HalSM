...
HalSMInteger HalSMIntegerThousand={0,(ui8[]){3,232}};
...
HalSMVariable* HalSMCompiler_getNameFunction(HalSMCompiler* hsmc,HalStringFormatChar* l)
{
    ...
    for (ui64 i=0;i<ll;i++) {
        ...
        if (HSFCharCompare(s,ASCIIH('('))) {
            HalSMArray* out=HalSMArray_init(hsmc->memory);
            HalSMArray_add(hsmc->memory,out,HalSMVariable_init_str(hsmc->memory,HalSMArray_chars_to_str(hsmc->memory,name)));
...
HalSMVariable* HalSMCompiler_compileList(HalSMCompiler* hsmc,ui8 isConsole,HalSMVariable* module,HalSMArray* allLines) {
    ...
    HalSMArray* tabs=HalSMArray_init(hsmc->memory);
    ...
    ui64 prevTabsC=0;
    HalSMArray* functions=HalSMArray_init(hsmc->memory);
...
HalStringFormatChar* HalSMVariable_to_str(HalSMCompiler* hsmc,HalSMVariable* a,ui8 isToPrint)
{
    ...
    if (a->type==HalSMVariableType_int) {
        ...
        HalStringFormatChar* name=ConcatenateStrings(hsmc->memory,U8H("0x"),hsmc->stringlibrary->Decimal2HexStr((ui64)((HalSMFunctionC*)a->value)->func));
