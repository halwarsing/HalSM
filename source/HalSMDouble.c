...
{
    HalSMDouble* rez = HalSMDouble_FromUnsignedInteger(memory,0), *fact = HalSMDouble_FromUnsignedInteger(memory,1);
    ...
    for (int point_seen = 0;HSFCharCompare(*str,ASCIIH('\0'))==0; str++){
        ...
        if (d >= 0 && d <= 9){
            if (point_seen){HalSMDouble_DivSelf(memory,fact,&HalSMDoubleTen);}
            HalSMDouble_MulSelf(memory,rez,&HalSMDoubleTen);
            HalSMDoubled.a->value[3]=d;
            HalSMDouble_AddSelf(memory,rez,&HalSMDoubled);
        } else {break;}
    }
    return HalSMDouble_clear(memory,HalSMDouble_Mul(memory,rez,fact));
}
